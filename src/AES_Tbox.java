...
public class AES_Tbox {
    public static int[][][] roundKey = new int[11][4][4];
    public static int[][][] Te = new int[4][256][4];
    public static int[][][] Td = new int[4][256][4];
    ...
    public static void main (String[] args) {
        ...
        for (int i=0;i<256;i++) {
            for (int j=0;j<4;j++)
                System.out.printf("%02x ", Te[0][i][j]);
            System.out.printf("\n");
        }
        ...
        for (int i=0;i<4000000;i++) {
            //System.out.println(encryptT(text, key));
            System.out.println(decryptT(text, key));
            text = StringADD(text);
            key = StringADD(key);
        }
    ...
    public static String encryptT (String plaintext, String key) {
        int i, j, k;
        ...
        for (i=1;i<10;i++) {
            tem1 = ADD(Te[0][state[0][0]], ADD(Te[1][state[1][1]], ADD(Te[2][state[2][2]], ADD(Te[3][state[3][3]], roundKey[i][0]))));
            tem2 = ADD(Te[0][state[1][0]], ADD(Te[1][state[2][1]], ADD(Te[2][state[3][2]], ADD(Te[3][state[0][3]], roundKey[i][1]))));
            tem3 = ADD(Te[0][state[2][0]], ADD(Te[1][state[3][1]], ADD(Te[2][state[0][2]], ADD(Te[3][state[1][3]], roundKey[i][2]))));
            tem4 = ADD(Te[0][state[3][0]], ADD(Te[1][state[0][1]], ADD(Te[2][state[1][2]], ADD(Te[3][state[2][3]], roundKey[i][3]))));
            state = new int[][]{tem1, tem2, tem3, tem4};
        }

        //由于最后一轮运算不包含列混淆，故还需要使用传统的算法
    ...
    public static String decryptT (String ciphertext, String key) {
        int i, j, k;
        int[] tem1, tem2, tem3, tem4;
        int[][] state = new int[4][4];
        generateKeys(key);
        ...
        //需要注意的是这里要先对轮密钥进行逆向列混淆处理，这一步通过Sbox变换和Td表即可实现
        for (i=9;i>=1;i--) {
            tem1 = ADD(Td[0][byteSub(roundKey[i][0][0])], ADD(Td[1][byteSub(roundKey[i][0][1])], ADD(Td[2][byteSub(roundKey[i][0][2])], Td[3][byteSub(roundKey[i][0][3])])));
            tem2 = ADD(Td[0][byteSub(roundKey[i][1][0])], ADD(Td[1][byteSub(roundKey[i][1][1])], ADD(Td[2][byteSub(roundKey[i][1][2])], Td[3][byteSub(roundKey[i][1][3])])));
            tem3 = ADD(Td[0][byteSub(roundKey[i][2][0])], ADD(Td[1][byteSub(roundKey[i][2][1])], ADD(Td[2][byteSub(roundKey[i][2][2])], Td[3][byteSub(roundKey[i][2][3])])));
            tem4 = ADD(Td[0][byteSub(roundKey[i][3][0])], ADD(Td[1][byteSub(roundKey[i][3][1])], ADD(Td[2][byteSub(roundKey[i][3][2])], Td[3][byteSub(roundKey[i][3][3])])));
            roundKey[i] = new int[][]{tem1, tem2, tem3, tem4};
        }

        for (i=9;i>=1;i--) {
            tem1 = ADD(Td[0][state[0][0]], ADD(Td[1][state[3][1]], ADD(Td[2][state[2][2]], ADD(Td[3][state[1][3]], roundKey[i][0]))));
            tem2 = ADD(Td[0][state[1][0]], ADD(Td[1][state[0][1]], ADD(Td[2][state[3][2]], ADD(Td[3][state[2][3]], roundKey[i][1]))));
            tem3 = ADD(Td[0][state[2][0]], ADD(Td[1][state[1][1]], ADD(Td[2][state[0][2]], ADD(Td[3][state[3][3]], roundKey[i][2]))));
            tem4 = ADD(Td[0][state[3][0]], ADD(Td[1][state[2][1]], ADD(Td[2][state[1][2]], ADD(Td[3][state[0][3]], roundKey[i][3]))));
            state = new int[][]{tem1, tem2, tem3, tem4};
        }

        //最后一次没有逆向列混淆，还需要使用传统算法
    ...
    //以下是T盒的生成函数
    public static void generateTE () {
        for (int i=0;i<4;i++) {
            for (int j=0;j<256;j++) {
                if (i == 0)
                    Te[i][j] = new int[]{multiply(0x2, byteSub(j)), byteSub(j), byteSub(j), multiply(0x3, byteSub(j))};
                else if (i == 1)
                    Te[i][j] = new int[]{multiply(0x3, byteSub(j)), multiply(0x2, byteSub(j)), byteSub(j), byteSub(j)};
                else if (i == 2)
                    Te[i][j] = new int[]{byteSub(j), multiply(0x3, byteSub(j)), multiply(0x2, byteSub(j)), byteSub(j)};
                else if (i == 3)
                    Te[i][j] = new int[]{byteSub(j), byteSub(j), multiply(0x3, byteSub(j)), multiply(0x2, byteSub(j))};
                else
                    System.out.println("Generate Te Failed!");
            }
        }
    }
    ...
    public static void generateTD () {
        for (int i=0;i<4;i++) {
            for (int j=0;j<256;j++) {
                if (i == 0)
                    Td[i][j] = new int[]{multiply(0xe, invbytesub(j)),  multiply(0x9, invbytesub(j)), multiply(0xd, invbytesub(j)), multiply(0xb, invbytesub(j))};
                else if (i == 1)
                    Td[i][j] = new int[]{multiply(0xb, invbytesub(j)), multiply(0xe, invbytesub(j)),  multiply(0x9, invbytesub(j)), multiply(0xd, invbytesub(j))};
                else if (i == 2)
                    Td[i][j] = new int[]{multiply(0xd, invbytesub(j)), multiply(0xb, invbytesub(j)), multiply(0xe, invbytesub(j)),  multiply(0x9, invbytesub(j))};
                else if (i == 3)
                    Td[i][j] = new int[]{multiply(0x9, invbytesub(j)), multiply(0xd, invbytesub(j)), multiply(0xb, invbytesub(j)), multiply(0xe, invbytesub(j))};
            }
        }
    }
    ...
    public static String StringADD (String COUNT) {
